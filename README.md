...
Given this notion of a 'conscious self' as an attention agent, how can we understand the split between the conscious and unconcious mind? In vibe theory, we can think of them as two entirely separate attention agents that are each largely tuned in to different 'frequencies' (components) in the collective vibe, and where the 'unconscious mind' only communicates with the 'conscious mind' through what the 'conscious mind' calls 'feelings'. Indeed, taking inspiration from psychoanalysis, we could say that the id (the 'unconscious mind') has learned to 'hide' its 'communication frequencies' from the ego (the 'conscious mind') to 'protect itself' from meddling, because the ego can't stand to hear the things the id has to say and always tries to silence the id (silencing something can be thought of as generating a 'destructive interference' vibe to cancel it out?). This is why the 'id' only communicates with the 'ego' via vague feelings, and because the 'logic' leading up to the feelings was (intentionally) hidden from the ego by the id, the ego can experience the feelings as 'coming out of nowhere' and 'making no sense'. Not surprising given that all of the 'higher reasoning rules' (that are attached to the frequencies that the ego typically uses) can't be directly applied to the id mind, because those rules aren't dialed in to the frequencies that the id relies on for internal communication! The poor id has to develop its own reasoning tools to use with its communication channels, and it may not do a very good job of them...but don't blame the 'id' if the ego thinks the emotions are 'irrational'; the 'ego' didn't want to listen to the reasoning in the first place, and that is why the reasoning was hidden.

Another major drawback to having a big 'split' in communication frequencies (i.e. a drawback of shoving a lot of stuff into the 'unconscious mind') is that all the experiential frequencies that the unconscious mind is using are not being attended to at all by the ego! This limits not only the experience of the ego, but also limits the complexity of the internal dialogue that the ego could partake in.
